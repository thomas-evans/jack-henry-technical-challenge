...
export class YTSearchModule extends LitElement {
  ...
  private recordBookmark(item: ModifiedItem) {
    ...
    if (lsBookmarks !== null) {
      ...
      if (this.isBookmarked(item.id.videoId)) {
        const filteredBookmarks = updatedBookmarks.filter(value => value.id.videoId !== item.id.videoId);
    ...
    return html`
        <jh-card show-header-divider show-footer-divider padding="small">
            ...
            <div class="jh-card-footer" slot="jh-card-footer">
                <jh-tooltip position="top-right"
                            label="${this.isBookmarked(item.id.videoId) ? "Remove Bookmark" : "Bookmark Video"}">
                    <jh-button accessible-label="Bookmark Video" name="bookMark"
                               appearance="${this.isBookmarked(item.id.videoId) ? "primary" : "secondary"}" size="small"
                               icon-position="after" @click=${() => this.recordBookmark(item)}>
  ...
  private resultsTemplate() {
    switch (this._getYTSearchData.status) {
      ...
      case TaskStatus.COMPLETE:
        ...
        return html`
            ${this.results?.items.map((item) => this.cardTemplate(item))}
        `
  ...
  private noBookmarksTemplate(){
    return html`
        <p>No bookmarks yet. Search for videos and bookmark the ones you want by pressing the bookmark button ( <jh-button accessible-label="Bookmark Video" name="bookMark" appearance="secondary" size="small">
            <jh-icon-bookmark slot="jh-button-icon"></jh-icon-bookmark>
        </jh-button> ) under each video.</p>
  ...
  private bookMarksTemplate() {
    return html`
        <section id="bookmarks">
            ${this.bookmarks.length === 0 ? this.noBookmarksTemplate() : this.bookmarks.map((item) => this.cardTemplate(item))}
        </section>
    `
  }
    ...
    return html`
        <main>
            <header>
                ...
                <jh-switch label="View Bookmarks" checked=${this.showBookmarks || nothing} @jh-change=${()=> this.showBookmarks = !this.showBookmarks}></jh-switch>                
            </header>
            ${this.showBookmarks ?
                    ...
                    html`
                        <section id="search">
                            <search-yt
                                    @fireSearch=${async (e: FireSearchEvent) => await this._getYTSearchData.run(e.detail)}
                                    @clearAll=${async () => this.clearAll()}
                            ></search-yt>
                        </section>
                        <section id="sort">
                            ${this._getYTSearchData.status === TaskStatus.COMPLETE ? html`<sort-results @sort=${() => this.requestUpdate()}></sort-results>`: nothing}
                        </section>
                            <section id="results">
